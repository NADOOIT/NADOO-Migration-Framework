...
def migrate_project(project_path, project_index, target_folder, card_num):
    ...
    try:
        ...
        if os.path.exists(target_project_path):
            logging.warning(f"Project {project_name} already exists in SD-CARD-{card_num}. Skipping...")
...
def main():
    ...
    try:
        ...
        total_required_space = sum(get_directory_size(str(d)) for d in github_path.iterdir() if d.is_dir() and is_git_repo(str(d)))
