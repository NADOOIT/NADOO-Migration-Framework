...
def update_feed_manager(project_path: str) -> bool:
    ...
    try:
        ...
        for file_path in feed_manager_files:
            ...
            if 'manager.create_element(' in content and 'from nadoo_framework.feed_manager import add_element_to_feed' not in content:
            ...
            content = re.sub(
                r'def create_main_window\((.*?)\):',
                r'def get_main_window_element_for_\1:',
                content
            )
...
'''
                content = 'import os\n' + content
                content = content.replace('FeedManager.get_instance()', 'FeedManager.get_instance()\n' + base_path_setup)
