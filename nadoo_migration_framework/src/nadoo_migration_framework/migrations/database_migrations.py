...
class AddColumnMigration(DatabaseMigration):
    ...
    def __init__(
        self, table_name: str, column_name: str, column_type: str, default_value: Any = None
    ):
    ...
    def _up(self) -> None:
        """Add the new column."""
        alter_sql = (
            f"ALTER TABLE {self.table_name} ADD COLUMN {self.column_name} {self.column_type}"
        )
...
class CreateIndexMigration(DatabaseMigration):
    ...
    def check_if_needed(self) -> bool:
        ...
        try:
            self._cursor.execute(
                f"SELECT name FROM sqlite_master WHERE type='index' AND name=?", (self.index_name,)
            )
...
class ModifyForeignKeyMigration(DatabaseMigration):
    ...
    def check_if_needed(self) -> bool:
        ...
        try:
            ...
            for fk in foreign_keys:
                if (
                    fk[2] == self.referenced_table
                    and fk[3] == list(self.foreign_key.keys())[0]
                    and fk[4] == self.referenced_column
                ):
                    return False
            return True
        except Exception:
    ...
    def _down(self) -> None:
        ...
        if self._original_schema:
            # Save current data
            self._cursor.execute(
                f"CREATE TABLE {self.table_name}_backup AS SELECT * FROM {self.table_name}"
            )
            ...
            # Restore data
            self._cursor.execute(
                f"INSERT INTO {self.table_name} SELECT * FROM {self.table_name}_backup"
            )
