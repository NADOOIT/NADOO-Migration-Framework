...
class MigrationWindow(toga.MainWindow):
    def __init__(self, title, project_path, changes):
        ...
        title_label = toga.Label(
            'NADOO Migration Framework', style=Pack(font_size=20, padding=(0, 0, 5, 0))
        )
        subtitle = toga.Label(
            f'Project: {self.project_path}', style=Pack(font_size=14, padding=(0, 0, 10, 0))
        )
        ...
        self.start_button = toga.Button(
            'Start Migration', on_press=self.start_migration, style=Pack(padding=(0, 5))
        )
        self.cancel_button = toga.Button(
            'Cancel', on_press=self.cancel_migration, style=Pack(padding=(0, 5))
        )
    ...
    def _create_migration_steps(self):
        ...
        for change in self.changes:
            steps.append(
                MigrationStep(
                    change, f"Apply change: {change}", lambda c=change: self._apply_change(c)
                )
            )
    ...
    def _update_steps_display(self):
        ...
        for step in self.steps:
            ...
            status = toga.Label(
                "●",
                style=Pack(color=status_colors[step.status], font_size=16, padding=(0, 5, 0, 5)),
            )

            # Add step title
            title = toga.Label(step.title, style=Pack(padding=(0, 5, 0, 0)))
        ...
        try:
            for step in self.steps:
                ...
                try:
                    ...
                    for completed_step in reversed(
            ...
            if all(step.status == "completed" for step in self.steps):
                self.main_window.info_dialog(
                    "Migration Complete", "Project has been successfully migrated!"
                )
            elif not self.is_migrating:
                self.main_window.info_dialog(
                    "Migration Cancelled", "Migration process was cancelled."
                )
            else:
