...
class NADOOMigrationApp(toga.App):
    ...
    def startup(self):
        ...
        scan_button = toga.Button(
            'Scan Repositories', on_press=self.scan_repositories, style=Pack(padding=(0, 0, 0, 10))
        )
        ...
        self.projects_scroll = toga.ScrollContainer(content=self.projects_box, style=Pack(flex=1))
        ...
        try:
            ...
            for repo in repos:
                ...
                name_label = toga.Label(
                    project.project_name, style=Pack(flex=1, padding=(0, 10, 0, 0))
                )

                # Status
                status_label = toga.Label(project.status, style=Pack(flex=1, padding=(0, 10, 0, 0)))
                ...
                if project.needs_migration:
                    migrate_button = toga.Button(
                        'Migrate', on_press=lambda p=project: self.migrate_project(p)
                    )
            ...
            self.main_window.error_dialog('Scan Error', f'Failed to scan repositories: {str(e)}')
    ...
    def migrate_project(self, project):
        ...
        try:
            ...
            if self.main_window.question_dialog(
                'Confirm Migration', f'The following changes will be made:\n\n{changes}\n\nProceed?'
            ):
                # TODO: Implement actual migration
                self.main_window.info_dialog(
                    'Migration Complete', f'Successfully migrated {project.project_name}'
                )
            ...
            self.main_window.error_dialog(
                'Migration Error', f'Failed to migrate {project.project_name}: {str(e)}'
            )
