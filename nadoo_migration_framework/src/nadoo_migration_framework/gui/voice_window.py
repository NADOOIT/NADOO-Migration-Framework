...
class VoiceCommandWindow(toga.Window):
    def __init__(self, title: str, project_path: Path, migration_window):
        ...
        title_label = toga.Label(
            'Voice Command Interface', style=Pack(font_size=20, padding=(0, 0, 5, 0))
        )
        subtitle = toga.Label(
        ...
        self.status_label = toga.Label('Status: Not Listening', style=Pack(padding=(0, 5, 0, 0)))
        ...
        commands_label = toga.Label(
            'Available Commands:', style=Pack(font_size=16, padding=(0, 0, 5, 0))
        )
        commands = [
            "Start migration",
            "Cancel migration",
        ...
        for cmd in commands:
            cmd_label = toga.Label(f"• {cmd}", style=Pack(padding=(0, 0, 5, 20)))
            commands_box.add(cmd_label)

        # Add Ollama status
        self.ollama_status = toga.Label('Ollama Status: Checking...', style=Pack(padding=(10, 0)))
        main_box.add(self.ollama_status)

        # Add start/stop button
        self.toggle_button = toga.Button(
            'Start Listening', on_press=self.toggle_listening, style=Pack(padding=5)
        )
        ...
        self.close_button = toga.Button(
            'Close Window', on_press=self.close_window, style=Pack(padding=(0, 5), width=120)
        )
    ...
    def _setup_voice_commands(self):
        ...
        self.voice_manager.add_command(
            "cancel migration", lambda: self.migration_window.cancel_migration(None)
        )
        self.voice_manager.add_command("show status", self._show_status)
        self.voice_manager.add_command(
            "create backup", lambda: asyncio.create_task(self.migration_window._create_backup())
        )
        self.voice_manager.add_command("stop listening", self.stop_listening)
