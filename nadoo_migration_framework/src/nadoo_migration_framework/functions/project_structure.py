...
def is_package_root(path: Path) -> bool:
    ...
    """
    return any(
        (path / f).exists() for f in ['setup.py', 'pyproject.toml', 'setup.cfg', 'requirements.txt']
    )
...
def find_entry_points(root_dir: Path) -> List[Dict[str, str]]:
    ...
    patterns = ['main.py', 'app.py', 'run.py', 'cli.py', 'server.py']

    for pattern in patterns:
        for path in root_dir.rglob(pattern):
            entry_points.append(
                {'name': path.stem, 'path': str(path.relative_to(root_dir)), 'type': 'script'}
            )
