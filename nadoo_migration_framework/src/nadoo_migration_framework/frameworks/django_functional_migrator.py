...
class ClassToModuleTransformer(ContextAwareTransformer):
    ...
    def _transform_view_method(self, method: cst.FunctionDef) -> cst.FunctionDef:
        ...
        new_params = [param for param in method.params.params if param.name.value != 'self']

        # Update function name to include HTTP method
        new_name = f"{method.name.value}_view"

        return method.with_changes(
            name=cst.Name(new_name), params=method.params.with_changes(params=new_params)
        )
    ...
    def _transform_class_attribute(self, attr: cst.Assign) -> cst.Assign:
        ...
        return attr.with_changes(
            targets=[target.with_changes(value=target.value.upper()) for target in attr.targets]
        )
        ...
        return cst.FunctionDef(
            ...
            body=cst.IndentedBlock(
                [
                    ...
                    cst.parse_statement(
                        "if select_related: queryset = queryset.select_related(*select_related)"
                    ),
        ...
        return cst.FunctionDef(
            ...
            body=cst.IndentedBlock(
                [
                    ...
                    cst.IndentedBlock([cst.Return(cst.Constant(value=False))]),
                ]
            ),
        ...
        functions.append(
            cst.FunctionDef(
                ...
                body=cst.IndentedBlock(
                    [
                        cst.parse_statement(
                            f"if not validate_{model_name.lower()}_data(data): return None"
                        ),
