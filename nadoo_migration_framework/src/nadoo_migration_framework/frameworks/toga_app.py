    ...
    def _is_gui_method(self, node: ast.FunctionDef) -> bool:
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name) and any(
                toga_name in child.id
                for toga_name in ['toga', 'Window', 'Box', 'Button', 'TextInput']
            ):
                toga_usage = True
                break
        return toga_usage
    ...
    def leave_Import(self, original_node: cst.Import, updated_node: cst.Import) -> cst.Import:
        ...
        for name in original_node.names:
            ...
            if old_name in self.old_to_new:
                new_names.append(name.with_changes(name=cst.Name(self.old_to_new[old_name])))
            else:
...
class BriefcaseTogaMigrator:
    ...
    def _find_src_dir(self) -> Optional[Path]:
        ...
        patterns = ['src', 'src/*/src', '**/src']  # For multi-platform projects
    ...
    def analyze_view(self, file_path: Path) -> ViewTransformation:
        ...
        view_code = self._generate_view_code(
            extractor.class_name, extractor.gui_methods, extractor.imports
        )

        # Generate logic code
        logic_code = self._generate_logic_code(
            extractor.class_name, extractor.logic_methods, extractor.imports
        )
    ...
    def _generate_view_code(
        self, class_name: str, gui_methods: List[ast.FunctionDef], imports: List[ast.Import]
    ) -> str:
    ...
    def _generate_logic_code(
        self, class_name: str, logic_methods: List[ast.FunctionDef], imports: List[ast.Import]
    ) -> str:
