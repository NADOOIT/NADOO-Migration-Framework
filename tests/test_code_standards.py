...
def get_all_functions(module) -> list:
    ...
    """
    return [obj for name, obj in inspect.getmembers(module, inspect.isfunction)
            if obj.__module__.startswith('nadoo_migration_framework')]
...
def test_test_function_naming():
    ...
    allowed_prefixes = {
        'fixture_', 'create_', 'setup_', 'mock_', 'get_', 'temp_', 'helper_',
        'process_', 'calculate_', 'update_', 'verify_', 'find_', 'add_', 'run_',
        'start_', 'stop_', 'interrupt_', 'check_', '_', 'main', 'slow_',
        'matrix_', 'recursive_', 'nested_', 'mixed_', 'memory_', 'large_',
        'curry_', 'feed_', 'brain_', 'home_', 'startup_', 'send_', 'discover_',
        'shared_', 'sample_', 'is_', 'post_', 'complete', 'hide', 'remove_',
        'voice_', 'another_'
    }
    ...
    allowed_files = {
        'test_utils.py',
        'test_toga_import_migrations.py',
        'conftest.py'
    }
    ...
    for test_file in test_files:
        ...
        for match in pytest_funcs:
            ...
            if not func_name.startswith('test_'):
                violations.append(f"Pytest function {func_name} in {test_file.name} "
        ...
        for match in func_defs:
            ...
            violations.append(f"Function {func_name} in {test_file.name} "
