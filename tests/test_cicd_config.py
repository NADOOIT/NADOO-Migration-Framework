...
def test_github_workflows_exist():
    ...
    for workflow, required_content in required_workflows.items():
        ...
        for required_text in required_content:
            assert required_text.lower() in content.lower(), \
...
def test_github_workflow_ci_structure():
    ...
    steps = [step.get('name', '').lower().replace(' ', '-')
            for step in test_job.get('steps', [])]
    
    for required_step in required_steps:
        assert any(required_step in step for step in steps), \
...
def test_dependabot_config():
    ...
    assert required_ecosystems.issubset(ecosystems), \
