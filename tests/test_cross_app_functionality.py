...
import shutil
import json
from src.functions.get_function_discovery_paths import get_function_discovery_paths, find_function_in_discovery_paths
...
'''
    ...
    with open(os.path.join(app1_dir, 'src', 'app1', 'functions', 'create_test_element.py'), 'w') as f:
        f.write(test_element_func)
        
    with open(os.path.join(app2_dir, 'src', 'app2', 'functions', 'process_secondary_data.py'), 'w') as f:
    ...
    try:
        ...
        for data in test_data:
            ...
            if processed:
                ...
                element = manager.create_element('create_test_element', processed.get_card()['data'])
                if element:
                    elements.append(element)
        
        # Verify all elements were created
        assert len(elements) == len(test_data), "All elements should be created"
        assert len(mock_feed.elements) == len(test_data) * 2, "Both processed and final elements should be in feed"
