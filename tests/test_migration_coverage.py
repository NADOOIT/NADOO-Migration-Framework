...
'''
    ...
    result = subprocess.run(
        ['coverage', 'report'], env=env, cwd=str(project_path), capture_output=True, text=True
    )
...
def update_test_imports(project_path: Path) -> None:
    ...
    content = content.replace(
        'from functions.math_utils', f'from {project_name}.functions.math_utils'
    )
...
def test_migration_preserves_functionality(tmp_path):
    ...
    sys.path.insert(0, str(tmp_path / 'src'))
    module = __import__(
        f"{project_name}.functions.math_utils", fromlist=['calculate_factorial', 'is_prime']
    )
    ...
    # Compare results
    assert (
        pre_migration_results == post_migration_results
    ), "Function results changed after migration"
