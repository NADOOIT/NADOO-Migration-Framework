...
def test_speak(voice_manager):
    ...
    with patch.object(voice_manager.engine, 'say') as mock_say, \
         patch.object(voice_manager.engine, 'runAndWait') as mock_run:
...
def test_record_audio(voice_manager):
    ...
    with patch.object(voice_manager.audio, 'open', return_value=mock_stream), \
