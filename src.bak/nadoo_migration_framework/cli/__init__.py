...
@click.option('--report-format', type=click.Choice(['text', 'markdown', 'json']), default='text', help='Output format for compatibility report')
@click.option('--output', type=click.Path(), help='Save report to file instead of stdout')
def migrate(project_path: str, auto: bool, dry_run: bool, backup: bool, gui: bool, 
           strict: bool, skip_style: bool, report_format: str, output: Optional[str]):
    ...
    try:
        ...
        blocking_issues = [issue for issue in compatibility_check.issues 
...
@click.option('--report-format', type=click.Choice(['text', 'markdown', 'json']), default='text', help='Output format for compatibility report')
@click.option('--output', type=click.Path(), help='Save report to file instead of stdout')
@click.option('--categories', type=str, help='Comma-separated list of categories to check (e.g., "python,dependencies")')
def check(project_path: str, gui: bool, strict: bool, skip_style: bool, 
         report_format: str, output: Optional[str], categories: Optional[str]):
...
@click.option('--categories', type=str, help='Comma-separated list of categories to fix (e.g., "code-style,dependencies")')
@click.option('--report-format', type=click.Choice(['text', 'markdown', 'json']), default='text', help='Output format for fix report')
@click.option('--output', type=click.Path(), help='Save report to file instead of stdout')
def fix(project_path: str, auto: bool, categories: Optional[str], report_format: str, output: Optional[str]):
...
def generate_compatibility_report(check: CompatibilityCheck, format: str) -> str:
    ...
    if format == 'json':
        ...
        if check.changes:
            lines.extend([
                "Required Changes:",
                *[f"  - {change}" for change in check.changes],
                ""
            ])
    ...
    if format == 'json':
        ...
        for category, category_results in results.items():
            ...
            for result in category_results:
                ...
                lines.append(
                    f"| {status} | {result.message} | {result.details or 'N/A'} |"
                )
            lines.append("")
