...
class CompatibilityRequirements:
    ...
    required_files: List[str] = field(default_factory=lambda: ["pyproject.toml", ".nadoo/config.toml"])
    style_rules: dict = field(default_factory=lambda: {
        "max_line_length": 100,
        "indent_size": 4,
    ...
    def check_dependencies(self) -> List[CompatibilityIssue]:
        ...
        try:
            ...
            if "tool" in pyproject_data and "poetry" in pyproject_data["tool"]:
                python_constraint = pyproject_data["tool"]["poetry"].get("dependencies", {}).get("python", "")
                if not any(v in python_constraint for v in [self.requirements.min_python_version, "^3", ">=3"]):
                    issues.append(CompatibilityIssue(
                        category="Dependencies",
                        severity="warning",
