...
class CodeMetricsCollector(cst.CSTTransformer):
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> None:
        ...
        self.function_metrics[func_name] = {
            ...
            'has_type_hints': any(p.annotation for p in node.params.params if isinstance(p, cst.Param)),
    ...
    def visit_ClassDef(self, node: cst.ClassDef) -> None:
        ...
        self.class_metrics[class_name] = {
            ...
            'docstring': node.get_docstring() is not None,
            'docstring_length': len(node.get_docstring() or ''),
            'method_count': sum(1 for n in node.body.body if isinstance(n, (cst.FunctionDef, cst.AsyncFunctionDef))),
            'attribute_count': sum(1 for n in node.body.body if isinstance(n, cst.SimpleStatementLine) and any(isinstance(s, cst.Assign) for s in n.body)),
            'line_count': len(node.body.body) if isinstance(node.body, cst.IndentedBlock) else 1,
        }
    ...
    def analyze_complexity(self, node: cst.CSTNode) -> None:
        ...
        if isinstance(node, (cst.If, cst.While, cst.For, cst.Try)):
            self.complexity_metrics['branching_count'] = self.complexity_metrics.get('branching_count', 0) + 1
        ...
        # Count operators
            ...
            self.complexity_metrics['operator_count'] = self.complexity_metrics.get('operator_count', 0) + 1
